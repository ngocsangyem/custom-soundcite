...
(function (window, document, callback) {
  ...
  function load_popcorn(version, cb) {
    ...
    if (loaded_p) {
      ...
      script.src =
        'https://cdn.knightlab.com/libs/soundcite/latest/vendor/popcorn.min.js'; // no SSL version of this, will self-host
  ...
  var playTemplate =
    '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 8 8" preserveAspectRatio="xMidYMin slice" width="100%" style="overflow: visible"> <path d="M0 0v6l6-3-6-3z" transform="translate(1 1)"></path> </svg>';
  var pauseTemplate =
    '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 8 8" preserveAspectRatio="xMidYMin slice" width="100%" style="overflow: visible"><path d="M0 0v6h2v-6h-2zm4 0v6h2v-6h-2z" transform="translate(1 1)"></path></svg>';
  var rgb = normalize_background_color(SOUNDCITE_CONFIG.background_color);
  var opacity = SOUNDCITE_CONFIG.opacity || 0.5;
  if (rgb) {
    ...
    if (match_group) {
      ...
      if (match_group) {
        function hex_to_int(hex) {
          // make sure to handle single chars correctly
  ...
  function Clip(el) {
    ...
    this.state = this.parent.querySelector('.audio-button__state');
    this.icon = this.parent.querySelector('.audio-button__icons');
    this.durationEL = this.parent.querySelector('.audio-button__duration');
    this.iconPlay = SOUNDCITE_CONFIG.play || playTemplate;
    this.iconPause = SOUNDCITE_CONFIG.pause || pauseTemplate;
    this.iconResume = SOUNDCITE_CONFIG.resume || playTemplate;
    this.playing = false;
  ...
  Clip.prototype.sound_loaded = function () {
    this.parent.addEventListener('click', bind(this.click_handler, this));
  ...
  function PopcornClip(el) {
    ...
    this.sound.on(
      ...
      bind(function () {
        if (this.durationEL) {
          this.durationEL.textContent = this.time();
        }
        if (this.end === null) {
  ...
  PopcornClip.prototype.pause_sound = function () {
    this.changeState('resume', this.iconPlay);
    this.sound.pause();
    this.sound.off('timeupdate');
  };

  PopcornClip.prototype.stop_sound = function () {
    this.changeState('listen', this.iconPlay);
    this.sound.pause();
    this.sound.off('timeupdate');
  };
  ...
  PopcornClip.prototype._play_sound = function () {
    ...
    this.changeState('pause', this.iconPause);
    this.sound.play();
    this.playing = true;
  ...
  PopcornClip.prototype.changeState = function (state, iconTemplate) {
    if (this.state) {
      this.state.textContent = state;
    }
    if (this.icon) {
      this.icon.innerHTML = '';
      this.icon.innerHTML = iconTemplate;
    }
  };
  PopcornClip.prototype.time = function () {
